...
pub async fn run_click(
    ...
    if dry_run {
        dry(
            fmt,
            "click",
            serde_json::json!({"ref": ref_id, "sel": sel}),
        );
        return Ok(());
    }
    let transport = nissia_cdp::connect(port).await?;
    if let Some(css) = sel {
        nissia_core::action::click::execute_selector(&transport, css).await?;
        maybe_record("click", HashMap::from([("sel".into(), css.into())]));
    } else {
        let r = ref_id.expect("click requires ref or sel (validated in main)");
        nissia_core::action::click::execute(&transport, r).await?;
        maybe_record("click", HashMap::from([("ref".into(), r.into())]));
    }
...
pub async fn run_dismiss(port: u16, fmt: &str) -> Result<()> {
    ...
    maybe_record("dismiss", HashMap::new());
    if fmt == "json" {
        println!("{raw}");
    } else {
...
/// Reload the current page (human recovery when a site errors/half-loads) and
/// wait for the DOM to be ready.
pub async fn run_reload(port: u16, hard: bool, fmt: &str) -> Result<()> {
    let transport = nissia_cdp::connect(port).await?;
...
pub async fn run_key(port: u16, key_name: &str, fmt: &str) -> Result<()> {
    let transport = nissia_cdp::connect(port).await?;
    nissia_core::action::key::execute(&transport, key_name).await?;
