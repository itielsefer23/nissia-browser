...
const DISMISS_JS: &str = r#"(function(){var clicked=0,removed=0;
function hd(e){try{e.style.setProperty('display','none','important');e.style.setProperty('pointer-events','none','important');e.setAttribute('aria-hidden','true');}catch(_){}}
function docs(){var a=[document];try{[].forEach.call(document.querySelectorAll('iframe'),function(f){try{if(f.contentDocument)a.push(f.contentDocument);}catch(e){}});}catch(e){}return a;}
